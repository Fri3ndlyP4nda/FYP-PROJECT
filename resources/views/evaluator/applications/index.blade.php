    ...
    <div class="container eval-shell">
        ...
        <section class="table-card">
            <div class="table-card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                ...
                {{-- Queue Filters --}}
                <div class="queue-filters" style="display: flex; gap: 8px; background: #f3ebee; padding: 4px; border-radius: 10px;">
                    <button type="button" class="filter-btn active" data-filter="all" style="border: none; background: #ffffff; color: #8B1E3F; padding: 6px 14px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s; box-shadow: 0 2px 6px rgba(0,0,0,0.05);">
                        All ({{ $total }})
                    </button>
                    <button type="button" class="filter-btn" data-filter="APEL A" style="border: none; background: transparent; color: #7b7274; padding: 6px 14px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s;">
                        APEL A ({{ $applications->where('application_type', 'APEL A')->count() }})
                    </button>
                    <button type="button" class="filter-btn" data-filter="APEL C" style="border: none; background: transparent; color: #7b7274; padding: 6px 14px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s;">
                                            ...
                                            @if (!$isFinalized)
                                                ...
                                                @if ($submission)
                                                    <a href="{{ route('evaluator.assessment.grading.show', $submission->_id) }}"
                                                        class="btn btn-secondary btn-sm"
                                                        style="background-color: #f3ebee; color: #8B1E3F; border: 1px solid #8B1E3F;">
                                                        Grade Submission
                                                    </a>
                                                @else
                                                    <button type="button" class="btn btn-secondary btn-sm" disabled 
                                                        style="opacity: 0.65; cursor: not-allowed; border: 1px solid #d1d5db;" 
            ...
            filterButtons.forEach(button => {
                button.addEventListener('click', function () {
                    ...
                    filterButtons.forEach(btn => {
                        ...
                        btn.style.color = '#7b7274';
                        btn.style.boxShadow = 'none';
                    });
                    this.classList.add('active');
                    this.style.background = '#ffffff';
                    this.style.color = '#8B1E3F';
